...
// p2plib's protocol handshake, information about the peer representative of this client, such as its ID is available.
//
...
func (c *Client) send(nonce uint64, data []byte, length uint32) error {
	c.writerCond.L.Lock()

	//c.writeSingle(message{nonce: nonce, length: length, data: data})

	c.writerBuf = append(c.writerBuf, message{nonce: nonce, length: length, data: data})
...
func (c *Client) request(ctx context.Context, data []byte, length uint32) (message, error) {
	...
	if err := c.send(nonce, data, length); err != nil {
	...
	case msg = <-ch:
		if msg.nonce == 0 {
                        //fmt.Printf("receive ACK nonce %d\n",msg.nonce)
...
func (c *Client) handshakeInbound() {
	...
	// Send our Ed25519 ephemeral public key and signature of the message '.__p2plib_handshake'.

	signature := sec.Sign([]byte(".__p2plib_handshake"))
	...
	// Read from our peer their Ed25519 ephemeral public key and signature of the message '.__p2plib_handshake'.
	...
	if !peerPublicKey.Verify([]byte(".__p2plib_handshake"), UnmarshalSignature(data[SizePublicKey:SizePublicKey+SizeSignature])) {
...
func (c *Client) recvBulkHandleLoop(first_msg message, ch chan message) {
	var msg message
        dummy := make([]byte, 1)

	first_msg.data = append([]byte{}, first_msg.data...)
        //fmt.Printf("start bulk loop. receive first partial. legnth:%d\n",first_msg.length)

        if err := c.send(first_msg.nonce, dummy, 0); err != nil {
            // TODO : error handle
            fmt.Printf("Error send ACK nonce:%d length:%d dummy size:%d\n",first_msg.nonce,first_msg.length,len(dummy))
	    return
        }
        //fmt.Printf("sent ACK nonce:%d\n",first_msg.nonce)
	...
	for {
	    select {
		    ...
		    if msg.length == 1 {
                        //fmt.Printf("receive last partial bulk legnth:%d\n",msg.length)
			break
		    }
                    //fmt.Printf("receive partial bulk legnth:%d\n",msg.length)

		    //test remove ACK
		    // send ACK
	            if err := c.send(msg.nonce, dummy,0); err != nil {
		        // TODO : error handle
		        continue
	            }
		    //fmt.Printf("sent ACK nonce:%d\n",msg.nonce)
	    ...
	    c.requests_recv.deleteRequest(first_msg.nonce)

            first_msg.nonce=0
            //fmt.Printf("handle message nonce:%d size:%d\n",first_msg.nonce, len(first_msg.data))
	...
	for {
		...
		if err != nil {
			...
			break
                ...
                if msg.length > uint32(0) {
		// check if it is existing request
		if ch,_ := c.requests_recv.findRequest(msg.nonce); ch != nil {
		        //fmt.Printf("request exists. nonce:%d\n",msg.nonce)
			// if existing, add to channel
			ch <- msg
			continue
		}else{
		    //fmt.Printf("request doesnt exist. nonce:%d length:%d\n",msg.nonce,msg.length)
		    if msg.length > uint32(1) {
			// if new request and bulk, create new channel for this nonce
		        //fmt.Printf("receive first bulk request nonce:%d\n",msg.nonce)
	                //ch, _, err := c.requests_recv.nextNonce()
	                ch, _, err := c.requests_recv.registerNonce(msg.nonce)
	                if err != nil {
		            fmt.Printf("error to get next nonce current_nonce:%d\n",msg.nonce)
			    // TODO : error ACK return and continue
			    continue
	                }
	                go c.recvBulkHandleLoop(msg, ch)
