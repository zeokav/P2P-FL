    ...
    def init_model_config(self):
        ...
        if os.path.exists("fake_data") and os.path.exists("my_class_distr"):
            ...
            fake_data, my_class_distr = self.datasource.fake_non_iid_data(
                ...
                max_train=MAX_DATASET_SIZE_KEPT,
    ...
    def run_trainer(self):
        while not self.shut_down:
            if len(self.all_ids) >= 6 and self.sorted_ids[0] == self.cid and not self.is_training:
                ...
                print("***************** Sending Model from Leader ********************")
                for layer in self.global_model.model.layers:
                    print("***************Global Model Layer Weight in Leader************ : ", [np.std(x) for x in layer.get_weights()])
                print("Local vs Global")
                for layer in self.local_model.model.layers:
                    print("***************Local Model Layer Weight in Leader************ : ", [np.std(x) for x in layer.get_weights()])
                metadata = {
                    "op": "request_update",
                    "round": 1,
                    "model_json": self.global_model.model.to_json()
    ...
    def register_handles(self):
        ...
        def handle_update_request(data):
            ...
            if parsed_data['round']==1:
                ...
                abcde  = model_from_json(parsed_data['model_json'])
                self.local_model.model  = model_from_json(parsed_data['model_json'])
                '''
                import tensorflow.keras as keras
                self.local_model.model.compile(loss=keras.losses.categorical_crossentropy,
                optimizer=keras.optimizers.Adadelta(),
                metrics=['accuracy'])
                '''
                import numpy as np
                self.global_model.model = self.local_model.model
                for layer in abcde.layers:
                    print("***************Layer Weight on Update Request************ : ", [np.std(x) for x in layer.get_weights()])

                print("***************** Setting Local Model to be same as Global Model from Leader ********************")
                for layer in self.local_model.model.layers:
                    print("***************Layer Weight on Update Request************ : ", [np.std(x) for x in layer.get_weights()])
