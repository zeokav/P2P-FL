...
import (
	...
	"github.com/theued/p2plib/gossip"
...
func TestGossip(t *testing.T) {
	...
	nodes := make([]*p2plib.Node, 0, 16)
	overlays := make([]*kademlia.Protocol, 0, cap(nodes))

	seen := make(map[p2plib.PublicKey]struct{}, cap(nodes))
	...
	for i := 0; i < cap(nodes); i++ {
		...
		hub := gossip.New(overlay,
			gossip.WithEvents(
				gossip.Events{
					OnGossipReceived: func(sender p2plib.ID, data []byte) error {
						cond.L.Lock()
