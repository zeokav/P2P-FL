...
*/
		//fmt.Printf("Listening for incoming peers.\n bind_addr %s\n id_addr %s\n",addr.String(),n.id.Address)
		...
		for {
			...
			if !exists {
				go client.inbound(conn, addr)
                            ...
                            for _, protocol := range n.protocols {
                                if protocol.OnExistingConnection == nil {
                                    continue
                                }
                                protocol.OnExistingConnection(true, client)
		            }
			}
...
func (n *Node) SendMessage(ctx context.Context, addr string, msg Serializable, opcode ...byte) error {
	...
	return n.Send(ctx, addr, data, opcode...)
}
...
func (n *Node) RequestMessage(ctx context.Context, addr string, req Serializable, opcode ...byte) (Serializable,  error) {
	...
	data, err = n.Request(ctx, addr, data, opcode...)
	if err != nil {
		return nil, err
	}
...
func (n *Node) Send(ctx context.Context, addr string, data []byte, opcode ...byte) error {
	...
	c, err, exist := n.dialIfNotExists(ctx, addr, opcode...)
	if err != nil {
		return err
	}
	if exist{
                for _, protocol := range n.protocols {
                    ...
                    protocol.OnExistingConnection(false, c, opcode...)
                }
	}

	if(uint32(len(data)) >= n.maxRecvMessageSize) {
	    var msize uint32
	    msize=0
	    length=0
	    for msize < uint32(len(data)) {
		length++
		msize += n.maxRecvMessageSize-uint32(128)
	    }
	    //fmt.Printf("Bulk sending. data size: %d , node.maxRecvMessageSize : %d length : %d\n",len(data), n.maxRecvMessageSize, length)
		...
		if uint32(len(data)) - end < n.maxRecvMessageSize-uint32(128) {
		    end = uint32(len(data))
		}else {
		    end += (n.maxRecvMessageSize-uint32(128))
		}
		//fmt.Printf("send request length:%d start:%d , end:%d\n",i,start,end)
		if err := c.send(nonce, data[start:end], i); err != nil {
	    ...
	    c.requests.deleteRequest(nonce)

	    return nil
	 }else { // single unit size
	    if err := c.send(0, data, 0); err != nil {
		return err
	    }
	}
...
func (n *Node) Request(ctx context.Context, addr string, data []byte, opcode ...byte) ([]byte, error) {
        c, err, _ := n.dialIfNotExists(ctx, addr, opcode...)
	if err != nil {
		return nil, err
	}

	msg, err := c.request(ctx, data, 0)
	if err != nil {
		return nil, err
	}
...
func (n *Node) dialIfNotExists(ctx context.Context, addr string, opcode ...byte) (*Client, error, bool) {
	var err error
	var exist bool
	...
	for i := uint(0); i < n.maxDialAttempts; i++ {
		...
		if !exists {
			go client.outbound(ctx, addr, opcode...)
		        exist=false
		}else{
		    exist=true
		}
