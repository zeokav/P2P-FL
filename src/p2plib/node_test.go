...
func TestSendUnderMaxNumConnections(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		node, err := p2plib.NewNode(p2plib.WithNodeMaxOutboundConnections(1))
		assert.NoError(t, err)

		defer node.Close()

		node.Handle(func(ctx p2plib.HandlerContext) error {
			wg.Done()
			return nil
		})
...
func TestSendWithTwoPeersUnderMaxNumConnections(t *testing.T) {
	...
	a, err := p2plib.NewNode(p2plib.WithNodeMaxOutboundConnections(1))
	assert.NoError(t, err)

	defer a.Close()

	a.Handle(func(ctx p2plib.HandlerContext) error {
		wg.Done()
		return nil
	})
	...
	b.Handle(func(ctx p2plib.HandlerContext) error {
		wg.Done()
		return nil
	})
...
func TestIdleTimeoutServerSide(t *testing.T) {
	...
	a, err := p2plib.NewNode(p2plib.WithNodeIdleTimeout(50 * time.Millisecond))
	assert.NoError(t, err)
...
func TestIdleTimeoutClientSide(t *testing.T) {
	...
	b, err := p2plib.NewNode(p2plib.WithNodeIdleTimeout(50 * time.Millisecond))
	assert.NoError(t, err)
...
func TestWithNodeMaxRecvMessageSize(t *testing.T) {
	...
	a, err := p2plib.NewNode(p2plib.WithNodeMaxRecvMessageSize(1 << 20))
	assert.NoError(t, err)
	defer a.Close()
